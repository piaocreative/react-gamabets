...
const FullSigma = ({
  providerId,
  providerName,
  gameTypeName = "Full Sangam Digit",
}) => {
  ...
  const [bidPana, setBidPana] = useState("");
  ...
  const createBid = async (event) => {
    ...
    if (!gameTypeKey) {
      ...
      msg = "Please Enter Open Digit!!!";
    } else if (!bidPana) {
      msg = "Please Enter Close Digit!!!";
    } else if (!bidPoints) {
        ...
        <Grid container spacing={1}>
          ...
          <Grid item xs={8}>
            <TextField
              ...
              placeholder="Enter Pana"
              value={bidPana}
              onChange={(e) => setNumValue("bidPana", e.target.value)}
              //   onClick={handleOpenGameDetail}
              InputProps={{
                sx: {
                  borderRadius: "17px",
